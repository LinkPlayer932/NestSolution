...
const From = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://nest-solution-backend.vercel.app/api/contact", {
        method: "POST",
  ...
  const faqs = [
    {
      q: "What services does Nextsol Agency provide?",
      a: "We provide SEO, Social Media Ads, Website Development, Email Marketing, and more.",
    },
    {
      q: "How long does it take to see SEO results?",
      a: "Generally, businesses start seeing improvements within 3–6 months depending on competition.",
    },
    {
      q: "Do you offer custom marketing plans?",
      a: "Yes, we create tailored strategies based on your goals and industry.",
    },
    {
      q: "Can I book a free consultation?",
      a: "Yes, fill out the form and our team will contact you shortly.",
    },
  ];

  return (
    <div
      className={`${sora.className} container mx-auto flex flex-col lg:flex-row items-start justify-center gap-12 p-5 md:p-10 lg:p-20`}
    >
      {/* LEFT SIDE */}
      <div className="w-full lg:w-1/2">
          ...
          {faqs.map((item, i) => (
            <div key={i} className="border border-gray-300 rounded-lg">
              ...
              >
                ...
                <span className="text-red-700 text-xl">{openFAQ === i ? "-" : "+"}</span>
              </button>

              {openFAQ === i && <div className="px-4 pb-4 text-gray-700">{item.a}</div>}
            </div>
          ))}
        </div>
      ...
      <div className="w-full lg:w-1/2 bg-white shadow-xl shadow-gray-400 p-6 md:p-10 rounded-2xl">
        ...
        <form className="w-full space-y-4" onSubmit={handleSubmit}>
          ...
          {successMessage && (
            <p className="mt-2 text-green-600 font-medium text-sm">{successMessage}</p>
          )}
        </form>
      </div>
    </div>
